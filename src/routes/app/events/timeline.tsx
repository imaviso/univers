								...
								<Popover>
									<PopoverTrigger asChild>
										<Button
											...
											className={cn(
												"w-fit justify-start text-left font-normal",
												!timelineSelectedDateRange && "text-muted-foreground",
											)}
										>
											<CalendarIcon className="mr-2 h-4 w-4" />
											{timelineSelectedDateRange?.from ? (
												timelineSelectedDateRange.to ? (
													<>
														...
														-{" "}
														{format(timelineSelectedDateRange.to, "LLL dd, y")}
													</>
												) : (
													format(timelineSelectedDateRange.from, "LLL dd, y")
												)
											) : (
												<span>Pick a date range</span>
											)}
									...
									<PopoverContent className="w-auto p-0" align="end">
										<Select
											onValueChange={(value) => {
												...
												if (value === "custom") {
													...
													setTimelineSelectedDateRange(undefined);
												} else if (value === "today") {
													setTimelineSelectedDateRange({
														from: startOfDay(now),
														to: endOfDay(now),
										...
										<Calendar
											initialFocus
											mode="range"
											defaultMonth={timelineSelectedDateRange?.from}
											selected={timelineSelectedDateRange}
											onSelect={setTimelineSelectedDateRange}
											numberOfMonths={1}
										/>
									</PopoverContent>
								</Popover>
					...
					<main className="flex-1 p-6 overflow-hidden">
						...
						{view === "list" ? (
							...
							<EventTimeline
								startDateISO={
									timelineSelectedDateRange?.from
										? formatISO(startOfDay(timelineSelectedDateRange.from))
										: undefined
								}
								endDateISO={
									timelineSelectedDateRange?.to
										? formatISO(endOfDay(timelineSelectedDateRange.to))
										: undefined
								}
							/>
						)}
