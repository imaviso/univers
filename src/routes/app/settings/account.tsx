	...
	return new Promise((resolve, reject) => {
		...
		image.onload = () => {
			...
			canvas.toBlob(
				(blob) => {
					if (!blob) {
						toast.error("Failed to create blob from cropped image.");
...
export function AccountSettings() {
	...
	const updateProfileMutation = useMutation({
		...
		onError: (err, _payload, context) => {
			if (context?.previousUserData) {
				...
				setProfileForm(getInitialProfileState(context.previousUserData));
			}
			toast.error(
	...
	const handleOtpSubmit = async () => {
		...
		if (!result.success) {
			setOtpError(
				v.flatten(result.issues).nested?.code?.[0] || "Invalid code format.",
			);
			return;
		}
		setOtpError(null);
	...
	const handleSetNewPasswordSubmit = async () => {
		...
		try {
			await userResetPassword(user.email, otpCode, result.output.newPassword); // Set new password
			...
			<Card>
					...
					<div className="flex flex-col items-center sm:flex-row sm:items-start gap-6">
						...
						<div className="relative group">
							...
							{isNewAvatarStaged && ( // Show remove button only for newly staged avatar
								...
								>
									<X className="h-4 w-4" />
									<span className="sr-only">Remove selected avatar</span>
								</Button>
							)}
							<input
								{...getInputProps()} // Use props from useFileUpload
								className="hidden" // Keep hidden as per hook's design
								disabled={updateProfileMutation.isPending || isCropperOpen}
							/>
						</div>
						{/* Details Section */}
				...
				<CardFooter className="flex justify-end gap-2 border-t pt-4">
					<Button
						...
						disabled={!profileHasChanges || updateProfileMutation.isPending}
					>
						Cancel
					</Button>
					<Button
			...
			>
				<DialogContent className="sm:max-w-md">
					...
					{passwordDialogStep === "request" && (
						<>
							<DialogHeader>
								<DialogTitle>Requesting Verification Code</DialogTitle>
						...
						<>
							...
							<div className="space-y-4 py-4">
								...
								<div className="grid gap-2">
									...
									{newPasswordErrors?.nested?.confirmPassword && (
										<p className="text-sm text-destructive">
											{newPasswordErrors.nested.confirmPassword[0]}
										</p>
									)}
					...
					{uncroppedPreviewUrl && (
						<div className="h-96 sm:h-120 bg-muted/30">
							...
							>
								...
								<CropperDescription>Avatar Cropping Area</CropperDescription>
							</Cropper>
						</div>
					)}
