...
type UpdateProfileInput = {
	...
	data: Partial<
		Pick<
			UserType,
			"firstName" | "lastName" | "phoneNumber" | "idNumber" | "departmentId"
		>
	>;
	imageFile?: File | null;
};
...
export const deactivateUser = async (userId: string) => {
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred during deactivating user.");
	}
};
...
export const getAllApprovalsOfEvent = async (
	...
	try {
		const response = await fetch(`${API_BASE_URL}/event-approval/${eventId}`, {
			method: "GET",
			headers: { "Content-Type": "application/json" },
			credentials: "include",
		});
...
export const getOwnEvents = async (): Promise<Event[]> => {
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred during fetching own events.");
	}
};
...
export const getPendingVenueOwnerEvents = async (): Promise<Event[]> => {
	try {
		const response = await fetch(`${API_BASE_URL}/events/pending/venue-owner`, {
			method: "GET",
			headers: { "Content-Type": "application/json" },
			credentials: "include",
		});
...
export const updateVenue = async ({
	...
	try {
		...
		const response = await fetch(`${API_BASE_URL}/admin/venues/${venueId}`, {
...
export const getNotifications = async (
	...
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred while fetching notifications.");
	}
};
...
export const createVenueReservation = async ({
	...
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred creating the reservation.");
	}
};
...
export const getAllReservations = async (): Promise<VenueReservationDTO[]> => {
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred fetching all reservations.");
	}
};
...
export const getOwnReservations = async (): Promise<VenueReservationDTO[]> => {
	try {
		...
		throw error instanceof Error
			? error
			: new Error("An unexpected error occurred fetching your reservations.");
	}
};
