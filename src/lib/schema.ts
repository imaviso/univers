...
export const personalInfoSchema = v.object({
	...
	departmentPublicId: v.pipe(v.string(), v.nonEmpty("Department is required")),
});
...
export const eventSchema = v.pipe(
	v.object({
		...
		venuePublicId: v.pipe(
			v.string("Venue is required"),
			v.nonEmpty("Venue is required"),
...
export const editEventSchema = v.pipe(
	v.object({
		eventName: v.optional(
			v.pipe(v.string(), v.nonEmpty("Event Name cannot be empty if provided")),
		),
		eventType: v.optional(
			v.pipe(v.string(), v.nonEmpty("Event Type cannot be empty if provided")),
		),
...
export const venueSchema = v.object({
	...
	venueOwnerId: v.pipe(
		...
		v.uuid("Venue Owner ID is required"),
	),
	image: v.optional(
		v.pipe(
...
export const setNewPasswordSchema = v.pipe(
	v.object({
		newPassword: v.pipe(
			...
			v.regex(/[a-z]/, "Your new password must contain a lowercase letter."),
			v.regex(/[A-Z]/, "Your new password must contain an uppercase letter."),
...
export const equipmentDataSchema = v.object({
	...
	categoryIds: v.optional(
		v.array(v.pipe(v.string(), v.uuid("Category ID must be a valid UUID"))),
	),
});
