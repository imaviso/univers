	...
	React.useEffect(() => {
		const getCurrentElementTime = () => {
			...
			for (let j = 0; j <= 1; j++) {
				...
				if (selected) {
					const trueIndex =
						activeIndex === -1 ? formattedHours * 2 + j : activeIndex;
						...
						return Array.from({ length: 2 }).map((_, part) => {
							...
							return (
								<li
									...
									onClick={() => handleClick(hour, part, PM_AM, trueIndex)}
									onKeyDown={(e) => {
	...
	const formateSelectedDate = React.useCallback(
		(selectedDate: Date | undefined) => {
			...
			if (Time?.includes(":") && Time?.includes(" ")) {
				...
				const period = minuteAndPeriodPart.substring(3, 5).toUpperCase(); // "PM"
