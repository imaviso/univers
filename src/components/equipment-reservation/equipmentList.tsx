...
export default function EquipmentList({
    ...
    const handleCategoryCheckboxChange = (
        ...
        const availableItemsInThisCategory =
            categoryGroup.equipmentItems.filter((item) => item.availability);
        ...
        if (newCheckedStatus) {
            ...
            for (const equipToAdd of availableItemsInThisCategory) {
                if (
                    !currentSelection.some(
                        (item) =>
                            item.equipmentId === equipToAdd.uniqueFrontendId,
                    )
                ) {
            ...
            const uniqueIdsToDeselect = new Set(
                availableItemsInThisCategory.map(
                    (item) => item.uniqueFrontendId,
                ),
            );
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    {groupedEquipment.map((group) => {
                        ...
                        let categoryCheckedState: boolean | "indeterminate" =
                            false;
                        if (itemsAvailableInCategory.length > 0) {
                            if (
                                ...
                                selectedAvailableItemsInCategory.length > 0
                            ) {
                                    ...
                                    <TableCell className="px-3 py-2">
                                        <Checkbox
                                            ...
                                            disabled={
                                                itemsAvailableInCategory.length ===
                                                0
                                            } // Disable category checkbox if no items are available
                                        />
                                    </TableCell>
                                    <TableCell
                                        colSpan={3}
