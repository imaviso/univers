...
export function TableFilterMenuItem<TData>({
    column,
    setProperty, // Removed 'table' as it wasn't used in the original snippet
}: {
    column: Column<TData>;
    // table: Table<TData>; // Remove if not needed elsewhere in the component
    setProperty: (value: string) => void;
}) {
    // 1. Remove the non-null assertion (!)
    const Icon = column.columnDef.meta?.icon;
    const displayName = column.columnDef.meta?.displayName; // Also good to get display name safely

    return (
        <CommandItem onSelect={() => setProperty(column.id)} className="group">
            <div className="flex w-full items-center justify-between">
                <div className="inline-flex items-center gap-1.5">
                    {/* 2. Conditionally render the Icon only if it exists */}
                    {Icon && <Icon strokeWidth={2.25} className="size-4" />}
                    {/* Render display name, fallback to column id if missing */}
...
>({
    ...
    function handleOptionSelect(value: string, check: boolean) {
        if (check) {
            ...
            column.setFilterValue(
                (old: undefined | FilterValue<"multiOption", TData>) => {
                    if (!old?.values[0] || old.values[0].length <= 1)
                        return undefined;

                    const newValues = [
                        uniq([...old.values[0], value]).filter(
                            (v) => v !== value,
                        ),
                    ];
        ...
        <Command loop>
            ...
            <CommandList>
                <CommandGroup>
                    {options.map((v) => {
                        const checked = Boolean(
                            filter?.values[0]?.includes(v.value),
                        );
                        const count = optionsCount[v.value] ?? 0;
...
export function PropertyFilterNumberValueMenu<TData, TValue>({
    ...
    const changeNumber = (value: number[]) => {
        ...
        column.setFilterValue(
            (old: undefined | FilterValue<"number", TData>) => {
                if (!old || old.values.length === 0) {
                    return {
                        operator: "is",
                        values: sortedValues,
